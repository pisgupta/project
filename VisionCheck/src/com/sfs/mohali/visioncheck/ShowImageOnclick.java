...
public class ShowImageOnclick extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		image.setImageBitmap(ImageCaptureCallback.rImg);
		...
		ImageCaptureCallback.rImg = null;
		retake.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_DOWN:
					...
					flag = true;
					ImageCaptureCallback.counter--;

					break;
				case MotionEvent.ACTION_UP:
					retake.setTypeface(null, Typeface.NORMAL);
					retake.setTextSize(17);
					CameraTest.surfaceView.setEnabled(true);
					CameraTest.iv.setImageBitmap(null);
					for (File fi : allfile) {
						String name = fi.getName();
						if (name.equals("Picture"
								+ ImageCaptureCallback.counter + ".png")) {
							fi.delete();
						}
					}
		...
		ok.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					...
					ok.setTextSize(17);
					if (ImageCaptureCallback.counter < 6) {
						Intent intent = new Intent(ShowImageOnclick.this,
								CameraTest.class);
						startActivity(intent);
					} else {
